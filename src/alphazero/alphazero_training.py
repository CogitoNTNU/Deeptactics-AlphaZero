...
class AlphaZero(Module):
    ...
    def vectorized_select(
        ...
        while (
            ...
            )  # TODO: Check if we can replace zeros_like with just 0.
    ...
    # @profile
    def run_simulation(
        self, state, neural_network: NeuralNetwork, move_number, num_simulations=800
    ):
        """
        ...
        actions = [child.action for child in root_node.children]
        normalized_root_node_children_visits = torch.tensor(
            [node.visits / (root_node.visits) for node in root_node.children],
            device=self.device,
            dtype=torch.float,
        )
...
def play_alphazero_game(
    alphazero_mcts: AlphaZero, nn: NeuralNetwork
...
def train_alphazero(num_games: int, epochs: int, batch_size: int = 1):
    ...
    try:
        ...
        for epoch in range(epochs):
            ...
            print([training_data[i] for i in indices])
            state_tensors, action_tensors, reward_tensors = zip(*[training_data[i] for i in indices])
            print("states")
            print(state_tensors)
            print("actions", action_tensors)
            print("rewardds:", reward_tensors)


            state_tensors = list(state_tensors)
            # (reward_tensors, _) = action_tensors
            # (reward_tensors, _) = reward_tensors
            print("num_samples:", num_samples, "batch_size:", batch_size)
            for i in range(0, num_samples, batch_size):
                batch_state_tensor = state_tensors[i : i + batch_size]
                batch_action_tensor = action_tensors[i : i + batch_size]
                batch_reward_tensor = reward_tensors[i : i + batch_size]
                print(f"Batch state tensor, {batch_state_tensor}")
                print(f"Batch state tensor size: {len(batch_state_tensor)}")

                optimizer.zero_grad()  # Reset gradients
                policy_pred, value_pred = nn.forward(batch_state_tensor[0])  # NB: DETTE ER MIDLERTIDIG OG VIL ØDELEGGE MED BATCH_SIZE > 1

                legal_actions_batch = []
                for state_tensor in batch_state_tensor:
                    # Flatten the tensor, find indices of nonzero elements, and convert them to a list
                    legal_actions = state_tensor.flatten().nonzero().squeeze().tolist()
                    # Add this list of legal actions to the batch list
                ...
                for i in range(len(legal_actions_batch)):
                    policy_pred = policy_pred[legal_actions_batch[i]].squeeze(
                        0
                    )  # Remove batch dimension
                    policy_pred = policy_pred[
                        legal_actions_batch[i]
                    ]  # Ensure that the policy_pred is a tensor, even if legal_actions is a single number.
                    batch_policy_pred.append(policy_pred)

                # Calculate loss
                value_loss = F.mse_loss(value_pred, batch_action_tensor)
                policy_loss = F.cross_entropy(policy_pred, batch_reward_tensor)
